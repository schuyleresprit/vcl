class CreateMap {
	...
	process_data(){
		...
		d3.keys(self.authors).forEach(function(d){
			if(!self.genres[d]){ self.genres[d] = []; }
		});
		d3.keys(self.data.genres).forEach(function(d){
			self.data.genres[d].forEach(function(_d){
				self.genres[_d.AuthorID].push(_d);
			});
		});
	}
	...
	generate_map(){
		...
		function filter_data(){
			...
			//make list of unique languages per place
			holder.forEach(function(d){
				d3.keys(d.value).forEach(function(_d){
			...
			holder.forEach(function(d){
				d3.keys(d.value).forEach(function(_d){
					d.value[_d].forEach(function(__d){
						if(!publications[__d.AuthorID]){ publications[__d.AuthorID] = []; }
						if(publications[__d.AuthorID].filter(function(t){ return t.PlaceID === __d.PlaceID && t.EndDate === __d.EndDate; }).length === 0){
							publications[__d.AuthorID].push(__d);
						}
					});
				});
			});
			...
			for(var i=0; i<d3.keys(publications).length; i++){
				for(var j=0; j<publications[d3.keys(publications)[i]].length -1; j++){
					publications[d3.keys(publications)[i]][j].PlaceID_End = publications[d3.keys(publications)[i]][j+1].PlaceID;
					publications[d3.keys(publications)[i]][j].Likelihood_End = publications[d3.keys(publications)[i]][j+1].Likelihood;
					publications[d3.keys(publications)[i]][j].tier = tier;
					tier=(j%2)*10;
				}
			}
			//make list of unique publications per place
			d3.keys(languages).forEach(function(d){
				if(!publications_unique[d]){ publications_unique[d] = []; }
			});
			d3.values(publications).forEach(function(d){
				d.forEach(function(_d){
					if(!publications_unique[_d.PlaceID]){ publications_unique[_d.PlaceID] = []; }
					if(!publications_unique[_d.PlaceID_End]){ publications_unique[_d.PlaceID_End] = []; }
					publications_unique[_d.PlaceID].push(_d);
					if(_d.PlaceID_End){ publications_unique[_d.PlaceID_End].push(_d); }
				});
			});
		}
		...
		function generate_sidebar(){
			...
			if(focus){
				...
				var data = sidebar_mode === 1 ? (languages_unique[focus.key] || []) : (publications_unique[focus.key]);
	...
	generate_routes(){
		...
		var starts = [],
				ends = [];
		d3.keys(self.genres).forEach(function(d){
			starts.push(d3.min(self.genres[d],function(_d){return (new Date(_d.StartDate));}));
			// ends.push(d3.max(self.genres[d],function(_d){return (new Date(_d.EndDate));}));
		});
		routes_scale.domain([
		...
		routes_lines_bknd = routes_g.selectAll('line.routes_lines_bknd')
			.data(function(d){ return self.genres[d]; });
