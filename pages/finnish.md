...
Represented here are the authors who have written works in Finnish. Some of these may be texts translated into Finnish from other languages.
...
<div class="col-sm-10">
<div class="page_title"><h3>Finnish</h3></div>
<html>
<body>
	<div class="container">
...
<h5>Return to <a href="{% link pages/languages.md %}">Languages</a></h5>
	...
	<script>
		...
		function showCategory (filter = "") {
			$('#data-container').html('');
			filter = filter.trim();
			$.getJSON("{{ site.baseurl }}/data/finnish.json", function (data) {
				let cards = [];
